...
If the A100 has no stock in `EUR-IS-1` (the volume's data center), preflight automatically tries the ordered fallback GPUs in `config/runpod-core.json`'s `pod.gpuFallbacks` (currently RTX PRO 6000 Blackwell, Server then Workstation Edition, both on Community Cloud up to $1.69/hour) and creates the single approved Pod with whichever candidate has stock within its own price cap. This never exceeds the saved $1.75/hour ceiling and never creates a second/replacement Pod. Report which GPU/cloud type was actually used (`preflight.json`'s `gpuTypeId`/`usedFallbackGpu`/`reliabilityNote`) — Community Cloud pods can be preempted by their host, a real reliability trade-off the owner accepted (2026-09-06) only because no Secure Cloud GPU in that data center matches the A100's VRAM class within the ceiling.

Run the launcher with `pwsh` (PowerShell 7), not Windows PowerShell 5.1 (`powershell.exe`) — it fails its own version check under 5.1 by design.
...
4. As soon as `startup-ui-ready.json` exists, open `http://127.0.0.1:8010` (or the v4.1 studio.html) and tell the owner the UI is available, with model loading/checks still pending. This is the owner's explicit UI-first preference; do not hold the interface behind inference tests. **Wire a live scrolling updater into the UI** (live-log feed or equivalent in the running UI) that streams real-time output from the pod's model load on the persistent volume ("Mounting volume...", "Weights from persistent storage...", progress/ETA, "Model LOADED"). The updater must be in the actual live UI, not wired to unrelated feeds. The UI automatically refreshes provider status and shows the live warmup; replies require the model server to finish loading, but background tests must not gate access. Report the fixed deadline and evidence path. Verify an actual UI exchange without disrupting the owner's conversation; an observed owner exchange counts. Report successful direct/wrapper checks separately. Do not terminate immediately after smoke tests unless requested or startup fails.
