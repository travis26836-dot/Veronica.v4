...
The Agents Run button includes **Start Veronica - 2 hours (PAID)**. Clicking this task is an explicit two-hour start request. It creates a fresh one-use approval record and then invokes `scripts/start-veronica.ps1`; the normal hourly ceiling, GPU and volume checks, availability checks, supervised watchdog, and bounded deadline remain in force. The task is manual-only and is not configured to run when a worktree is created.
